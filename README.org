...
*** Setup proxies
Unfortunately my proxy requires some additional arguments for curl ~-k~ or ~--ssl-no-revoke~. It is not possible to set those arguments for now:

#+begin_src emacs-lisp :tangle no
  (when (and gptel-use-curl (my/has-or-get-proxy))
    (setq gptel-proxy (my/has-or-get-proxy))
    ;;there is no such option at the moment
    (setq gptel-user-argument "-k"))
#+end_src

So here is the current solution: Use the ~url.el~
