...
HLSL code can be compiled in this way
#+begin_src shell
dxc -Zi -Qembed_debug -Fc /tmp/output.s -Tps_6_6 example.hlsl #this lacks of demangler
#+end_src

GLSL code can be compiled this way
#+begin_src shell
glslangValidator "-g" "--spirv-dis" "-o" /tmp/output.s example.glsl  
#+end_src

#+begin_src emacs-lisp
(defun rmsbolt--glsl-compile-cmd (&key src-buffer)
  ...
  (rmsbolt--with-local-files
   ...
   (let* ( ;; Turn off passing the source file if we find compile_commands
          ...
          (cmd (string-join
                (list cmd
                      "-g" "--spirv-dis"
                      "-o" output-filename src-filename)
                " ")))
     cmd)))
#+end_src
