...
** Curl TLS sanity check
Many packages shell out to ~curl~ (eca, gptel, ...) and all of them report a
broken certificate store badly -- ~eca~ feeds curl's stderr to its JSON parser
and complains about ~(json-parse-error 1 1 1)~, which tells you nothing.

Hit it once after startup and let curl speak for itself.
#+begin_src emacs-lisp
  (defun my/curl-tls-check ()
    "Warn if curl cannot verify TLS certificates."
    (interactive)
    (when-let* ((curl (executable-find "curl")))
      (with-temp-buffer
        ;; body goes to the null device, so the buffer collects only stderr
        (let ((code (call-process curl nil t nil "-sS" "-I" "--max-time" "5"
                                  "-o" null-device "https://github.com")))
          ;; 35/60/77/83 are certificate failures; 6/7/28 just mean no network
          (when (memq code (list 35 60 77 83))
            (warn "curl cannot verify TLS certificates (exit %s):\n%s"
                  code (string-trim (buffer-string))))))))

  ;; on idle so it never delays startup
  (add-hook 'emacs-startup-hook
            (lambda () (run-with-idle-timer 5 nil #'my/curl-tls-check)))
#+end_src
...
Disable changing font size by scrolling, using ~C-x C--~ and ~C-x C-+~ instead!!! Very easily meshed up with track-pad.
...
Without proper ~LC_CTYPE~ locales, you will get [[https://stackoverflow.com/questions/60557160/python3-8-fails-with-fatal-python-error-config-get-locale-encoding][config_get_locale_encoding error]] from python lsp servers.
...
#+begin_src emacs-lisp
  (use-package pomodoro :defer t
    :vc (:url "https://github.com/baudtack/pomodoro.el")
...
** Devil mode
[[https://susam.github.io/devil/][Devil Mode]] is a non-intrusive modifier-free editing experience, compared to evil mode or god mode.  With this, I can actually forget about the keyboard mapping problems across windows and Mac.
#+begin_src emacs-lisp
...
** Mac OS settings
See [[file:macos.org::+title: Personal Mac OS adjustments][MacOS adjustments]]
...
#+begin_src emacs-lisp
  (use-package ox-moderncv ;;TODO this is an exception to defer?
    :vc (:url "https://github.com/Titan-C/org-cv")
...
[[https://jblevins.org/projects/markdown-mode/][Markdown Mode for Emacs]] is popular with AI tools, Emacs has good support for it.
#+begin_src emacs-lisp
...
(add-hook 'server-after-make-frame-hook
	  (lambda () (progn
		       ...
		       ;; using dark theme on server
		       (setq appr-light-theme-name
			     (if (display-graphic-p) 'modus-operandi-tinted
			       appr-dark-theme-name))
		       (setq appr-dark-theme-hour 17)
		       (setq appr-light-theme-hour 8)
		       ;;This is a bit hacky at the moment. Need to apply the theme once
		       (appr-apply-theme)
		       )))
...
Normally this requires "HARFBUZZ" feature set but on Mac it seemed to work without it. You can verify by toggle global switch ~global-ligature-mode~.
...
(use-package ligature
  ...
  :if (or (eq window-system 'ns)
          (string-match "HARFBUZZ" system-configuration-features))
...
(use-package ssh-agency
  :vc (:url "https://github.com/magit/ssh-agency")
...
(use-package treesit-auto
    :unless (or (eq system-type 'windows-nt)
		(eq system-type 'gnu/linux)  ;;the treesitter is broken at the moment
...
(use-package eglot
  ...
  ;;eldoc sometimes pops up a separate *eldoc* doc buffer when the echo area
  ;;isn't big enough; force everything into the minibuffer instead
...
*** [[https://github.com/flycheck/flycheck-eglot][Enable Flycheck for eglot as opposed flymake]]

#+begin_src emacs-lisp
(use-package flycheck-eglot :ensure t :pin melpa
  :after (flycheck eglot)
  :config
  (global-flycheck-eglot-mode 1))
#+end_src

We should also disable flymake by ~(setq eglot-stay-out-of '(company flymake))~, see [[https://utcc.utoronto.ca/~cks/space/blog/programming/EmacsLspModeToEglot][this blog]].
...
#+end_src
*** Setup company and eglot
#+begin_src emacs-lisp :tangle no :noweb-ref COMPANY_HOOKS
((c++-mode c++-ts-mode c-mode c-ts-mode) . company-mode)
#+end_src

We exclude ~c-mode~ and ~c-ts-mode~ from ~eglot-ensure~ because ~slang-mode~ derives from ~c-mode~. When eglot starts clangd for a C/C++ buffer in a project, it registers the server in ~eglot--servers-by-project~ with managed modes including ~c-mode~. When a ~.slang~ file is opened in the same project, ~eglot--maybe-activate-editing-mode~ (on ~after-change-major-mode-hook~) calls ~eglot-current-server~, which finds clangd via ~eglot--servers-by-project~. The ~eglot--languageId~ predicate then checks ~provided-mode-derived-p~ — since ~slang-mode~ derives from ~c-mode~, it returns non-nil, so eglot thinks clangd can manage the slang buffer and activates ~eglot--managed-mode~ with clangd instead of slangd. Use ~c++-mode~ for C files.

#+begin_src emacs-lisp :tangle no :noweb-ref EGLOT_HOOKS
((c++-mode c++-ts-mode) . eglot-ensure)
#+end_src

#+begin_src emacs-lisp :tangle no :noweb-ref EGLOT_SERVERS
...
(use-package cppinsights
  ...
  :vc (:url "https://github.com/xeechou/cppinsights.el")
  :defer t
  ...
  (when (eq system-type 'darwin)
    ;; For this to get correct headers from system, we need additional options
    (setq cppinsights-clang-opts '("-isysroot" "/Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX.sdk"))
    )
)
#+end_src

*NOTE* that ~cppinsights.el~ does not work for projects!!! If a ~compile_commands.json~ exists, it will simply becomes ~(insights filename)~ I don't recall cppinsights itself has any supports to grab additional arguments from ~compile_commands.json~.
...
(use-package slang-mode
  ...
  :vc (:url "https://github.com/K1ngst0m/slang-mode")
  :defer t
  :mode (("\\.slang\\'" . slang-mode)
         ("\\.sl\\'" . slang-mode)
         ("\\.slangh\\'" . slang-mode))
  :init
  (defun my/slang-no-electric-indent-for-brackets (char)
    "Prevent electric-indent from reindenting on [ and ] in `slang-mode'."
    (when (memq char '(?\[ ?\]))
      'no-indent))
  ...
  ;; disable indentation for [] syntax
  (slang-mode . (lambda ()
		  (add-hook 'electric-indent-functions
			    #'my/slang-no-electric-indent-for-brackets
			    nil
			    t)))
  ;; disable format-all
  (slang-mode . (lambda () (fmo-mode -1)))

  (slang-mode . eglot-ensure)
  )
#+end_src

Do not use the buggy slang-lsp.el, we just setup the server here. The actual configuration need to go to ~eglot-workspace-configuration~ in ~.dir-locals.el~.

#+begin_src emacs-lisp :tangle no :noweb-ref EGLOT_SERVERS
(when (executable-find "slangd")
  (add-to-list 'eglot-server-programs
	       `((slang-mode) . (,(executable-find "slangd")))))
#+end_src
...
#+begin_src emacs-lisp :tangle no :noweb-ref RMSBOLT_COMPILERS
...
Both ~slangc~ and ~glslangValidator~ compile commands support additional arguments via buffer-local variables. This is useful for passing project-specific flags (e.g. include paths, defines) from ~.dir-locals.el~.
...
(put 'rmsbolt-slang-extra-args 'safe-local-variable #'my/rmsbolt-extra-args-safe-p)
#+end_src

For example, in a project's ~.dir-locals.el~:
#+begin_src emacs-lisp :tangle no
((slang-mode . ((rmsbolt-slang-extra-args . ("-I./shaders/include" "-DUSE_BINDLESS=1"))))
 (glsl-mode  . ((rmsbolt-glsl-extra-args  . ("-I./shaders/include")))))
#+end_src

**** compiler
Then we use ~slangc~ to compile to SPIR-V assembly with debug info (~-target spirv-asm -g2~). 
...
Finally we can define a front-end.
...
*** TODO Formatter setup
This does not work for now because 1: ~(language-id-buffer)~ does not take slang, 2: format-all does not take "SLANG" as language either. I need to make a PR. 3: clang-format's hlsl mode is WIP.
...
*** RMSBOLT support
Let's allow extra args in the loop
#+begin_src emacs-lisp :tangle no :noweb-ref RMSBOLT_COMPILERS
...
(put 'rmsbolt-glsl-extra-args 'safe-local-variable #'my/rmsbolt-extra-args-safe-p)  
#+end_src

#+begin_src emacs-lisp :tangle no :noweb-ref RMSBOLT_COMPILERS
(defun my/rmsbolt-glsl-compile-cmd (&key src-buffer)
...
** GLSL-fx
#+begin_src emacs-lisp
(use-package glslfx-mode
  :defer t :load-path "lisp/"
  :mode ("\\.glslfx\\'" . glslfx-mode))
#+end_src
...
#+begin_src emacs-lisp :noweb yes
  ...
  (use-package hlsl-mode
    :vc (:url "https://github.com/xeechou/hlsl-mode.el")
    :defer t
    :mode (("\\.fxh\\'"    . hlsl-mode)
           ("\\.hlsl\\'"   . hlsl-mode)
           ("\\.vs\\'"     . hlsl-mode)
...
#+begin_src emacs-lisp
  (use-package azsl-mode
    :vc (:url "https://github.com/xeechou/azsl-mode.el")
    :defer t
    :mode (("\\.azsl\\'"  . azsl-mode)
           ("\\.azsli\\'" . azsl-mode))
...
** Python
*** company mode :: add ~python-ts-mode~, we use  ~company-capf~ by language servers.
#+begin_src emacs-lisp :tangle no :noweb-ref COMPANY_HOOKS
...
*** Setup [[https://journal.spencerwnelson.com/entries/emacs_virtualenv_lsp.html][Python virtualenvs]]
...
In the ~.dir-locals.el~, it can be setup like this
#+begin_src emacs-lisp :tangle no
...
** LEAN
#+begin_src emacs-lisp
(use-package lean4-mode :ensure t :defer t
  :commands lean4-mode
...
The org-babel is supported, see [[file:lisp/ob-lean4.el][ob-lean4.el]] and ~org-funcs-load-babel-compiler~ at [[file:lisp/org-funcs.el][org-funcs.el]]

*** installing elan 
[[https://lean-lang.org/install/manual/][installing lean]] on Linux and Mac is through package manager
#+begin_src bash :tangle no
brew install elan-init #on Mac
paru -S elan-init #on Arch
#+end_src

On windows is not very straight-forward, since they just recommend using VsCode, but it is still doable
#+begin_src bash :tangle no
...
#remeber to add $HOME/.elan/bin to $PATH
#+end_src

*** using lake, the package manager
~lake~ is the lean package manager, you can use it create project and compile it
#+begin_src bash :tangle no
lake new /path/to/project
cd /path/to/project && lake build
#+end_src

Or after cloning the project, retrieve the packages
#+begin_src bash :tangle no
...
#+end_src
*** Mathlib
creating a project depend on mathlib
#+begin_src bash :tangle no
lake +leanprover-community/mathlib4:lean-toolchain new mathliblean-tutorial math  
#+end_src

Updating the math-lib, somehow need to update it manually for now
#+begin_src bash :tangle no
...
(use-package jenkinsfile-mode
  :after groovy-mode
  :vc (:url "https://github.com/john2x/jenkinsfile-mode")
...
** [[https://aider.chat/][Aider]] (disabled in favor of eca)
...
**** Claude
#+begin_src emacs-lisp :tangle no :noweb-ref AIDER_BACKENDS
(defun my/aider-use-claude-architect ()
...
*** Tools
gptel [[https://github.com/karthink/gptel/wiki/Tools-collection][wiki]] has a collection of tools, some of them are listed below, you can also check out [[https://github.com/skissue/llm-tool-collection][llm-tool-collection]] repo.
**** [[https://github.com/skissue/llm-tool-collection][skissue/llm-tool-collection]]

#+begin_src emacs-lisp
(use-package llm-tool-collection :ensure t
  :vc (:url "https://github.com/skissue/llm-tool-collection" :rev :newest))
#+end_src

Then integrate with gptel
#+begin_src emacs-lisp  :tangle no :noweb-ref GPTEL_TOOLS
...
**** local AI
Define your own Open-AI API compatible AI providers like these
#+begin_src emacs-lisp :tangle no
(setq my/localai-providers
      '((:name "provider 1"
               :host  "https://provider1.url"
               :endpoint "/v1/chat/completions"
               :stream t
               :key nil) ;;this one does not need any key
        (:name "alt"
               :host "https://provider2.url"
               :endpoint "/v1/chat/completions"
               :stream t
               :key "sk-aabb-ccdd-eeff-gghh")))
#+end_src

Here we setup all of them for gptel

#+begin_src emacs-lisp :tangle no :noweb-ref GPTEL_BACKENDS
(defun my/gptel-update-localai ()
  "Updating local-ai providers, you might want to do this more often when you
changed VPN"
  (interactive)
  
  (when (boundp 'my/localai-providers)
    (dolist (provider my/localai-providers)
      ;;test if reachable in case if you need VPN
      (when (localai-reachable-p (plist-get provider :host) nil (plist-get provider :key))
	(gptel-make-openai
            (plist-get provider :name)
          :host     (substring (plist-get provider :host) (length "https://"))
          :protocol "https"
          :endpoint (plist-get provider :endpoint)
          :key      (if (plist-get provider :key) (plist-get provider :key) "dummy")
          :stream   (plist-get provider :stream)
          :models   (localai-get-models-sync
                     (plist-get provider :host) nil (plist-get provider :key)))))))
;;Now run it for the first time
(my/gptel-update-localai)
#+end_src

**** Copilot chat
#+begin_src emacs-lisp :tangle no :noweb-ref GPTEL_BACKENDS
(when (boundp 'my/llm-copilot-key)
...
** [[https://github.com/editor-code-assistant/eca-emacs][editor code assistant]]
ECA is LSP protocol for llms
#+begin_src emacs-lisp
(use-package eca :ensure t :pin melpa
  ...
  (when (boundp 'my/llm-deepseek-key)
    (setenv "DEEPSEEK_API_KEY" my/llm-deepseek-key))
  )
#+end_src

*** Tools
ECA can call external tools, most likely it is some kind of command line tool. For example, this tool use ~trafilatura~ to download the web page and convert that into markdown files.
...
(use-package openwith
  :vc (:url "https://github.com/garberw/openwith" :rev "master")
