...
#+begin_src emacs-lisp :noweb yes
  (use-package gptel
    ...
    :config
    ;;setup directives
    (dolist (prompt '(<<GPTEL_DIRECTIVES>>))
...
#+begin_src emacs-lisp :tangle no :noweb-ref GPTEL_DIRECTIVES
  (cmd-helper . "You are a command line helper. Generate command line that do what requested. Without any additional description or explaination. Generate ONLY the command, I will edit before running")
#+end_src
