...
#+begin_src emacs-lisp :noweb yes
  ...
  (use-package hlsl-mode
    ...
    :hook ((hlsl-mode . company-mode)
           ;;TODO there is some bug on windows that :config block always run. I have to put it in hook
           (hlsl-mode . (lambda ()
                          <<HLSL_LSP>>
                          )))
    )
  ...
  (if (and (executable-find "slangd")
      ...
      (progn
        ...
        (add-to-list 'eglot-server-programs
                     `(hlsl-mode . ("slangd")))
        (add-to-list
         (make-local-variable 'company-backends) 'company-capf))
    ;; not available
    (my/merge-list-to-list
     (make-local-variable 'company-backends)
...
#+begin_src emacs-lisp
  (use-package rust-mode
    :ensure t :defer t :mode (("\\.rs\\'" . rust-mode)))
#+end_src
...
#+begin_src emacs-lisp
  ...
  ;; built-in
  (use-package nxml-mode :mode (("\\.xml\\'" . nxml-mode)
                                ("\\.ofx\\'" . nxml-mode)
                                ;;quick book
                                ("\\.qfx\\'" . nxml-mode)))
#+end_src
